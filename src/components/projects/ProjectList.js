...
const ProjectCard = memo(({ project, index }) => {
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      transition={{ duration: 0.4, ease: "easeOut", delay: Math.min(index * 0.1, 0.3) }}
      className="w-full flex flex-col bg-white dark:bg-black/40 backdrop-blur-lg border border-gray-200 dark:border-white/10 rounded-3xl overflow-hidden group hover:border-black/30 dark:hover:border-white/30 transition-all duration-300 relative shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-[0_8px_30px_rgba(0,0,0,0.2)] dark:hover:shadow-[0_8px_30px_rgba(0,0,0,0.4)] will-change-transform"
    >
      ...
      {/* Image section */}
      <div className="w-full relative bg-gray-50 dark:bg-white/5 border-b border-gray-200 dark:border-white/10 overflow-hidden transform-gpu">
        <div className="block relative">
          {project.image && project.image[0] && (
            <Image
              ...
              width={800}
              height={450}
              className={`w-full h-auto object-cover transition-transform duration-500 group-hover:scale-[1.02] ${project.darkImage ? 'dark:hidden' : ''} will-change-transform`}
              quality={70}
          ...
          {project.darkImage && project.darkImage[0] && (
            <Image
              ...
              width={800}
              height={450}
              className="w-full h-auto object-cover transition-transform duration-500 group-hover:scale-[1.02] hidden dark:block will-change-transform"
              quality={70}
              sizes="(max-width: 768px) 100vw, 50vw"
            />
          )}
      ...
      {/* Content section */}
      <div className="p-4 md:p-6 flex flex-col flex-1 transform-gpu">
        {/* Header section with Title and Links */}
        <div className="flex flex-row justify-between items-start mb-4 gap-4">
          <h2 className="text-lg md:text-2xl font-semibold md:font-medium text-black dark:text-white transition-colors duration-300">
            {project.title}
          </h2>
